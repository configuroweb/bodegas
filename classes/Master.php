...
Class Master extends DBConnection {
	...
	function save_pricing(){
		...
		if($save){
			$resp['status'] = 'success';
			if(empty($id))
				$this->settings->set_flashdata('success',"Precio de Bodega exitosamente ingresado");
			else
				$this->settings->set_flashdata('success',"Precio de bodega actualizado correctamente.");
		}else{
			$resp['status'] = 'failed';
			$resp['err'] = $this->conn->error."[{$sql}]";
		}
	...
	function delete_pricing(){
		...
		if($del){
			$resp['status'] = 'success';
			$this->settings->set_flashdata('success',"Precio de bodega eliminado correctamente.");
		}else{
			$resp['status'] = 'failed';
			$resp['error'] = $this->conn->error;
		}
	...
	function save_tenant(){
		...
		if($check > 0){
			$resp['status'] = 'failed';
			$resp['msg'] = "Arrendatari@ existe actualmente";
			return json_encode($resp);
			exit;
		}
		if(empty($id)){
		...
		if($save){
			$resp['status'] = 'success';
			if(empty($id))
				$this->settings->set_flashdata('success',"Nuev@ arrendatari@ registrad@ exitosamente");
			else
				$this->settings->set_flashdata('success',"Arrendatari@ actualizad@ exitosamente");
		}else{
			$resp['status'] = 'failed';
			$resp['err'] = $this->conn->error."[{$sql}]";
		}
	...
	function delete_tenant(){
		...
		if($del){
			$resp['status'] = 'success';
			$this->settings->set_flashdata('success',"Arrendatari@ eliminad@ exitosamente");
		}else{
			$resp['status'] = 'failed';
			$resp['error'] = $this->conn->error;
		}
	...
	function save_rent(){
		...
		if($save){
			$resp['status'] = 'success';
			if(empty($id))
				$this->settings->set_flashdata('success',"Nuevo alquiler guardado correctamente.");
			else
				$this->settings->set_flashdata('success',"Alquiler actualizado correctamente.");
	...
	function delete_rent(){
		...
		if($del){
			$resp['status'] = 'success';
			$this->settings->set_flashdata('success',"Alquiler eliminado correctamente.");
		}else{
			$resp['status'] = 'failed';
			$resp['error'] = $this->conn->error;
		}
