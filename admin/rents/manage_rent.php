...
<div class="card card-outline card-info">
	<div class="card-header">
		<h3 class="card-title"><?php echo isset($id) ? "Actualizar detalles de alquiler": "Alquiler Nuevo" ?> </h3>
	</div>
	<div class="card-body">
		<form action="" id="rent-form">
			...
			<label for="tenant_id">Arrendatari@</label>
			...
			<div class="col-md-6 form-group">
				<label for="unit_id"># Bodega</label>
			...
			<div class="form-group col-md-6">
				<label for="rent_type" class="control-laberl">Tipo</label>
				<select name="rent_type" id="rent_type" class="form-control form-control-sm" required>
					<option value="1" <?php echo isset($rent_type) && $rent_type == 1 ? 'selected' : '' ?>>Mensual</option>
					<option value="2" <?php echo isset($rent_type) && $rent_type == 2 ? 'selected' : '' ?>>Quincenal</option>
					<option value="3" <?php echo isset($rent_type) && $rent_type == 3 ? 'selected' : '' ?>>Anual</option>
				</select>
			</div>
			<div class="form-group col-md-6">
				<label for="billing_amount" class="control-laberl">Monto</label>
			...
			<div class="form-group col-md-6">
				<label for="date_rented" class="control-laberl">Fecha Alquiler</label>
			...
			<div class="form-group col-md-6">
				<label for="" class="control-laberl">Estado</label>
	...
	<div class="card-footer">
		<button class="btn btn-flat btn-primary" form="rent-form">Guardar</button>
		<a class="btn btn-flat btn-default" href="?page=rents">Cancelar</a>
	</div>
</div>
<script>
		...
		$('#rent_type,#unit_id').on('keyup input change',function(){
			...
			$.ajax({
				...
				success:function(resp){
					...
					alert_toast("Se produjo un error al obtener el precio.","error")
					}
				}
			})
		})
        $('.select2').select2({placeholder:"Selecciona aquí",width:"relative"})
		$('#rent-form').submit(function(e){
			e.preventDefault();
            var _this = $(this)
			 $('.err-msg').remove();
			 if($('#schedule_id').val() <= 0){
				alert_toast(" Es necesario agendar",'warning')
				$('#schedule_id').focus()
				return false;
			}
			start_loader();
